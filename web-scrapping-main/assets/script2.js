...
(async () => {
  ...
  await page.type(".search-global-typeahead__input", "corporate");
  await page.keyboard.press("Enter");
  ...
  await filterValues("Add an industry", "professional services");
  await clickBtns();
...
async function openCompanyPages(companyCount) {
  ...
  for (let i = 0; i < companiesList.length; i++) {
    ...
    try {
      ...
      console.log(error);
      if(page1 != undefined)  await page1.close();
    }
  }
  if (!isNextBtnDisabled && companyCount < 500) {
    await nextBtn.click();
    await page.waitForNavigation();
...
// to click the show result btns of locations, companySize and fields
async function clickBtns(){
    await page.waitForSelector('div[visible]');
    let val = await page.$('div[visible]')
    let btn = await val.$('button:nth-child(2)');
    await btn.click()
    await page.waitForNavigation()
}
